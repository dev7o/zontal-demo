    ...
    <div class="container-zon between items-center px-2 flex">
        <div class="flex items-center w-full">
            ...
            <ul class="flex ml-12">
                ...
                <li><a class="block px-4 text-sm text-gray-500 dark:text-gray-100"
                        href="<?php AutoPlay() ?>">AutoPlay</a></li>
            </ul>
            <?php // Search Bar Form ?>
            <form action="<?php echo $site_url ?>search" class="ml-6 bg-gray-100 search-bar rounded-full flex py-3">
                ...
                <input name="query" class="outline-none text-sm text-gray-700 text-light bg-transparent w-64"
                    type="text" placeholder="Search Your Favourite Game">
            </form>
            ...
            <?php if (isset($_SESSION['Loggedin'])) { ?>
                <?php if ($_SESSION['Loggedin'] == true) { ?>
                    ...
                    <div class="profile relative flex h-10 mr-20">
                        <img onclick="document.querySelector('.profile-dropdown').classList.toggle('hidden')"
                            src="<?php echo $site_url ?>static/img/<?php User_Data('user_pic') ?>"
                            class="h-10 cursor-pointer w-10 rounded-full mr-3 ">
                        <div onclick="document.querySelector('.profile-dropdown').classList.toggle('hidden')"
                        ...
                        <div
                            class="profile-dropdown hidden py-2 rounded-lg w-52 bg-white dark:bg-zinc-800 absolute left-0 top-[50px]">
                            <ul class="w-full">
                                <?php
                                if (isset($_SESSION['is_admin_Loggedin'])) { ?>
                                    <li><a href="<?php echo $site_url ?>admin/index"
                                            class="block py-2 px-3 dark:text-gray-300 text-sm">Admin Panel</a></li>
                                    <div class="divider w-full h-0.5 border-t mt-2 mb-2 border-gray-200 dark:border-zinc-700"></div>
                                <?php } ?>
                                <li><a href="<?php echo $site_url ?>user/likes"
                                        class="block py-2 px-3 dark:text-gray-300 text-sm">Liked Games</a></li>
                                <li><a href="<?php echo $site_url ?>user/comments"
                                        class="block py-2 px-3 dark:text-gray-300 text-sm">Comments</a></li>
                                <div class="divider w-full h-0.5 border-t mt-2 mb-2 border-gray-200 dark:border-zinc-700"></div>
                                <li><a href="<?php echo $site_url ?>user/settings"
                                        class="block py-2 px-3 dark:text-gray-300 text-sm">Settings</a></li>
                                <li><a href="<?php echo $site_url ?>logout"
                                        class="block py-2 px-3 dark:text-gray-300 text-sm">Logout</a></li>
                            </ul>
                        </div>
                    </div>
...
<?php // Mobile Header  ?>
<div
    class="mobile-header sticky mb-6 hidden bg-white shadow-lg dark:bg-zinc-900 items-center flex justify-between w-full">
...
<div class="offcanvas fixed top-0 left-0 h-[100%]  w-full">
    ...
    <div class="menu-list bg-white dark:bg-zinc-900 offcanvas-menu">
        <div
            class="close-offcanvas text-black dark:text-white cursor-pointer absolute font-bold px-4 py-4 select-none top-0 right-0 text-black text-lg ">
            &times;</div>
        ...
        <ul class="w-full h-full overflow-y-scroll scroll-hidden ">
            <li><a class="block px-4 py-2 uppercase text-xs text-gray-600 dark:text-gray-200"
            ...
            <?php if (!isset($_SESSION['Loggedin'])) { ?>
                ...
                <li><a class="block px-4 py-2 uppercase text-xs text-gray-600 dark:text-gray-200"
                        href="<?php echo $site_url ?>register">signup</a></li>
            <?php } ?>
            <?php if (isset($_SESSION['Loggedin'])) { ?>
                <?php if ($_SESSION['Loggedin'] == true) { ?>
                    <li><a class="block px-4 py-2 uppercase text-xs text-gray-600 dark:text-gray-200"
                            href="<?php echo $site_url ?>user/admin/index">Admin Panel</a></li>
                    <li><a class="block px-4 py-2 uppercase text-xs text-gray-600 dark:text-gray-200"
                            href="<?php echo $site_url ?>user/likes">liked games</a></li>
                    <li><a class="block px-4 py-2 uppercase text-xs text-gray-600 dark:text-gray-200"
                            href="<?php echo $site_url ?>user/comments">comments</a></li>
                    <li><a class="block px-4 py-2 uppercase text-xs text-gray-600 dark:text-gray-200"
                            href="<?php echo $site_url ?>user/settings">settings</a></li>
                    <li><a class="block px-4 py-2 uppercase text-xs text-gray-600 dark:text-gray-200"
                            href="<?php echo $site_url ?>logout">logout</a></li>
                <?php }
            } ?>
        </ul>
    </div>
