            ...
            <div class="flex justify-between flex-md-column items-center">
                ...
                <ul class="flex">
                    ...
                    while ($row = mysqli_fetch_assoc($run)) { ?>
                        ...
                        <li><a class="block px-3 text-xs text-gray-400"
                                href="page/<?= $row['id'] ?>/<?php echo $revspace; ?>"><?= $row['title'] ?></a></li>
                    <?php } ?>
                    <?php // Pages Loop End 
                    ?>
                </ul>
...
<script>
    $("#game_like").on("click", () => {
        ...
        var game_id = <?php if (isset($_GET['id']))
            return Game_Data(Secure_DATA($_GET['id']), 'id'); ?>;
        $.ajax({
            type: "POST",
